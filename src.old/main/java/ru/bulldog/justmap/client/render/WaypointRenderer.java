...
public class WaypointRenderer {
	...
	private void renderHUD(Waypoint waypoint, Minecraft minecraft, float delta, float fov, int dist) {
	...
	private void renderWaypoint(PoseStack matrixStack, VertexConsumerProvider consumerProvider, Waypoint waypoint, Minecraft client, Camera camera, float tick, int dist) {
		int wpX = waypoint.pos.getX();
	...
	private void renderIcon(PoseStack matrixStack, VertexConsumer vertexConsumer, float[] colors, float alpha, int size) {
	...
	private void renderLightBeam(PoseStack matrixStack, VertexConsumer vertexConsumer, float tick, int i, int j, float[] colors, float alpha, float h, float k) {
		int m = i + j;
	...
	private void renderBeam(PoseStack matrixStack, VertexConsumer vertexConsumer, float red, float green, float blue, float alpha, int j, int k, float l, float m, float n, float o, float p, float q, float r, float s, float t, float u, float v, float w) {
		PoseStack.Entry entry = matrixStack.peek();
		Matrix4f matrix4f = entry.getModel();
		Matrix3f matrix3f = entry.getNormal();		
