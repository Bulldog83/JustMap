...
public class WaypointEditor extends MapScreen {
	...
	public void init() {
		...
		this.nameField = new TitledButtonWidget<>(font, new TextFieldWidget(font, 0, 0, ew - 30, 12, new TextComponent("Name")), ex, ey, ew, rowH, "", lang("name").asString());
		...
		this.xField = new TextFieldWidget(font, px, ey, ew, rowH, new TextComponent(""));
		this.xField.setTextPredicate(validNumber);
		this.xField.setMaxLength(7);
		this.xField.setText(waypoint.pos.getX() + "");
		
		this.yField = new TextFieldWidget(font, px + ew, ey, ew, rowH, new TextComponent(""));
		this.yField.setTextPredicate(validNumber);
		this.yField.setMaxLength(7);
		this.yField.setText(waypoint.pos.getY() + "");
		
		this.zField = new TextFieldWidget(font, px + 2 * ew, ey, ew, rowH, new TextComponent(""));
		...
		ew = 20;
		this.prevColorButton = new ButtonWidget(ex, ey, ew, rowH, new TextComponent("<"), (b) -> cycleColor(-1));
		this.children.add(prevColorButton);
		
		this.nextColorButton = new ButtonWidget(x + screenW - ew - padding, ey, ew, rowH, new TextComponent(">"), (b) -> cycleColor(1));
		this.children.add(nextColorButton);
		
		ey += row;
		
		this.prevIconButton = new ButtonWidget(ex, ey, ew, rowH, new TextComponent("<"), (b) -> cycleIcon(-1));
		this.children.add(prevIconButton);
		
		this.nextIconButton = new ButtonWidget(x + screenW - ew - padding, ey, ew, rowH, new TextComponent(">"), (b) -> cycleIcon(1));
		...
		this.children.add(new SliderWidget(elemX, ey, sliderW, rowH, TextComponent.EMPTY, (double) this.showRange / SHOW_RANGE_MAX) {
			{
				this.updateMessage();
			}
	...
	private void borderedRect(PoseStack matrixStack, int x, int y, int w, int h, int color, int border, int borderColor) {
		int hb = border >> 1;
