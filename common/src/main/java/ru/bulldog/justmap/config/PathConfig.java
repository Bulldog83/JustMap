...
public class PathConfig extends Config {
	...
	@Nullable
	public <T, E extends Entry<T>> E getRootEntry(String key, Class<E> type) {
		return this.getEntry(createKey(key), type);
	}
	...
	public <T extends Enum<T>, E extends EnumEntry<T>> T getEnum(String key, T defaultValue, Class<E> type) {
		return getEnum(createKey(key), defaultValue, type);
	}

	@Nullable
	public <T extends Enum<T>, E extends EnumEntry<T>> T getEnum(String key, Class<E> type) {
		return getEnum(createKey(key), type);
	}
