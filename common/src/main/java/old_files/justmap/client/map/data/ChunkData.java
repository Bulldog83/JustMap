...
public class ChunkData {
	...
	public ChunkData(WorldData data, LevelChunk lifeChunk) {
	...
	public ChunkData(WorldData data, ChunkPos pos) {
		...
		if (Dimension.isOverworld(world)) {
			ServerLevel serverWorld = DataUtil.getServerWorld();
			if (serverWorld != null) {
...
//			}
	...
	private void initLayer(Layer layer) {
		int levels = this.world.getMaxBuildHeight() / layer.height;
		...
		if (color != -1) {
			...
			if (ClientSettings.showTopography) {
				return MathUtil.isOdd(chunkLevel.sampleHeightmap(index)) ?
						ColorUtil.colorBrightness(color, -0.6F) : color;
			}
			return color;
		}
		return Colors.BLACK;
